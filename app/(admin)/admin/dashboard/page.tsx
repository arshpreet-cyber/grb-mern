...
function StatCard({ title, value, change, bg, text, pillBg, isDown, href, subtext = "vs last month" }: { title: string; value: string | number; change: string; bg: string; text: string; pillBg: string; isDown?: boolean; href?: string; subtext?: string }) {
  // Extracting the tailwind background class to derive a dark mode background
  const darkBg = "dark:bg-slate-800/40";
  const isActuallyDown = isDown !== undefined ? isDown : change.startsWith('-');
  ...
  const content = (
    <div className={`relative overflow-hidden rounded-[16px] bg-white dark:bg-[#1a1f2c] border border-[#f0f0f0] dark:border-slate-800 shadow-[0_4px_12px_rgba(0,0,0,0.02)] flex flex-col justify-between transition-all hover:shadow-md ${href ? 'cursor-pointer hover:border-gray-300 dark:hover:border-slate-600' : ''}`}>
      ...
      <div className={`px-5 py-3 flex items-center justify-between ${bg} ${darkBg}`}>
        <span className="text-[12px] text-gray-500 dark:text-white font-medium tracking-wide">{subtext}</span>
        <div className={`flex items-center gap-1.5 rounded-[6px] px-2.5 py-0.5 text-white ${pillBg}`}>
          {isActuallyDown ? <TrendingDown size={14} strokeWidth={2.5} /> : <TrendingUp size={14} strokeWidth={2.5} />}
...
function DashboardSkeleton() {
  return (
    <div className="space-y-6 min-h-screen animate-pulse">
      {/* Hello Banner Skeleton */}
      <div className="relative rounded-[20px] bg-gradient-to-r from-gray-100 to-gray-200 dark:from-slate-850/30 dark:to-slate-900/30 px-8 py-8 h-[132px] border border-[#f0f0f0] dark:border-slate-800 flex flex-col justify-center gap-2">
        <div className="h-9 w-64 bg-gray-250 dark:bg-slate-700 rounded-lg"></div>
        <div className="h-5 w-96 bg-gray-200 dark:bg-slate-800 rounded-lg mt-1"></div>
      </div>

      {/* Stats Cards Skeleton */}
      <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5">
        {[1, 2, 3, 4, 5].map((i) => (
          <div key={i} className="rounded-[16px] bg-white dark:bg-[#1a1f2c] border border-[#f0f0f0] dark:border-slate-800 p-5 flex flex-col justify-between h-[136px]">
            <div>
              <div className="h-4 w-24 bg-gray-255 dark:bg-slate-750 rounded"></div>
              <div className="h-8 w-32 bg-gray-200 dark:bg-slate-700 rounded mt-3"></div>
            </div>
            <div className="h-8 w-full bg-gray-100 dark:bg-slate-850 rounded mt-4"></div>
          </div>
        ))}
      </div>
      ...
      <div className="grid gap-6 xl:grid-cols-[2fr_1fr]">
        <div className="rounded-[16px] bg-white dark:bg-[#1a1f2c] border border-[#f0f0f0] dark:border-slate-800 p-6 h-[380px] flex flex-col justify-between">
          <div className="h-6 w-36 bg-gray-255 dark:bg-slate-750 rounded"></div>
          <div className="flex-1 bg-gray-100/60 dark:bg-slate-850/40 rounded mt-6"></div>
        </div>
        <div className="rounded-[16px] bg-white dark:bg-[#1a1f2c] border border-[#f0f0f0] dark:border-slate-800 p-6 h-[380px] flex flex-col justify-between">
          <div className="h-6 w-36 bg-gray-255 dark:bg-slate-750 rounded"></div>
          <div className="flex-1 bg-gray-100/60 dark:bg-slate-850/40 rounded mt-6"></div>
...
export default function AdminDashboard() {
  ...
  useEffect(() => {
    ...
    setApiError(null);
    fetch(`/api/dashboard/analytics?month=${encodeURIComponent(selectedMonth)}&year=${selectedYear}`)
      .then((r) => {
  ...
  return (
    <div className="relative space-y-6 min-h-screen animate-fade-slide">
        ...
        <div className="relative z-10 flex flex-col gap-6 sm:flex-row sm:items-center sm:justify-between">
          ...
          <div className="relative">
            ...
            >
              ...
              <span className="text-[16px] font-regular">{selectedMonth} {selectedYear}</span>
            ...
            {isMonthDropdownOpen && (
              <>
                ...
                />
                <div className="absolute right-0 mt-2 w-72 rounded-2xl bg-white dark:bg-slate-900 shadow-[0_20px_50px_rgba(0,0,0,0.2)] border border-gray-100 dark:border-slate-800 p-3.5 z-20 animate-in fade-in zoom-in-95 duration-200">
                  {/* Year selector segment */}
                  <p className="text-[10px] font-bold text-gray-400 dark:text-slate-500 uppercase tracking-widest mb-1.5 pl-1">Select Year</p>
                  <div className="flex gap-1.5 mb-3 overflow-x-auto pb-2 scrollbar-thin scrollbar-thumb-gray-200">
                    {[2021, 2022, 2023, 2024, 2025, 2026].map((year) => (
                      <button
                        key={year}
                        onClick={() => setSelectedYear(year)}
                        className={`px-3 py-1.5 rounded-lg text-[12px] font-bold shrink-0 transition-all ${
                          selectedYear === year
                            ? "bg-[#fc0] text-slate-900 shadow-md scale-105"
                  ...
                  <p className="text-[10px] font-bold text-gray-400 dark:text-slate-500 uppercase tracking-widest mb-1.5 pl-1 border-t border-gray-100 dark:border-slate-850 pt-2.5">Select Month</p>
                  ...
                  <button
                    onClick={() => {
                      setSelectedMonth("Yearly");
                      setIsMonthDropdownOpen(false);
                    }}
                    className={`w-full mt-2.5 flex items-center justify-center rounded-xl py-2.5 text-[13px] font-semibold transition-all ${
                      selectedMonth === "Yearly"
                        ? "bg-[#111827] text-white dark:bg-[#fc0] dark:text-slate-900 shadow-lg scale-[1.02]"
                  ...
                  <div className="mt-3.5 border-t border-gray-50 dark:border-slate-800 pt-2 text-center">
                    <p className="text-[10px] font-bold text-gray-400 dark:text-slate-500 uppercase tracking-widest">Selected: {selectedMonth === "Yearly" ? "Yearly" : selectedMonth.slice(0, 3)} • {selectedYear}</p>
                  </div>
                </div>
              </>
            )}
      ...
      {showAnalytics && (() => {
        ...
        const subtext = selectedMonth === "Yearly" ? "Last Year" : "Last Month";

        return (
          <div className={isCurrentPeriod ? "grid gap-6 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5" : "grid gap-6 sm:grid-cols-2 xl:grid-cols-4"}>
            <StatCard title="Total Revenue" value={`$${revenue.allTime.toLocaleString()}`} change={changes.revenue} bg="bg-[#FCF3E8]" pillBg="bg-[#AF670F]" text="text-[#AF670F]" href="/admin/orders" subtext={subtext} />
            {isCurrentPeriod && (
              <StatCard title="Today's Revenue" value={`$${revenue.today.toLocaleString()}`} change={changes.todayRevenue} bg="bg-[#EBF7EE]" pillBg={changes.todayRevenue.startsWith('-') ? "bg-rose-600" : "bg-[#1B8A43]"} text="text-[#1B8A43]" href="/admin/orders" subtext="Yesterday" />
            )}
            <StatCard title="Total Orders" value={stats.totalOrders.toLocaleString()} change={changes.orders} bg="bg-[#F2F6FE]" pillBg="bg-[#285FF5]" text="text-[#285FF5]" href="/admin/orders" subtext={subtext} />
            <StatCard title="Total Users" value={stats.totalUsers.toLocaleString()} change={changes.users} bg="bg-[#EEFAEB]" pillBg="bg-[#54CE12]" text="text-[#54CE12]" href="/admin/users" subtext={subtext} />
            <StatCard title="Total Tickets" value={(stats.openTickets + stats.closedTickets + stats.awaitingTickets).toLocaleString()} change={changes.tickets} bg="bg-[#F4EBFE]" pillBg="bg-[#9B52F0]" text="text-[#9B52F0]" href="/admin/tickets" subtext={subtext} />
          </div>
        );
      })()}
          ...
          <div className="space-y-8">
            {charts.topProducts.map((p, i) => (
              <div key={i} className="flex items-center gap-5">
                <div className="h-14 w-14 shrink-0 bg-[#FFF8F1] dark:bg-slate-900/50 rounded-xl flex items-center justify-center border border-[#FFECD6]/50 dark:border-slate-800 transition-colors overflow-hidden">
                  {p.image ? (
                    <img 
