...
function StatItem({ target, suffix, label, description, shouldAnimate }: any) {
...
export default function StatsBar({ data, settings }: SectionProps) {
  const {
  ...
  const styles: React.CSSProperties = {
    padding: settings.padding || '4rem 1rem',
    margin: settings.margin || '0',
    backgroundColor: settings.backgroundColor || '#ffffff',
  };

  return (
    <section style={styles} className="relative w-full font-sans" ref={sectionRef}>
      <Wrapper>
        <div className="w-full">
          <div className="text-center mb-16 md:mb-[80px]">
            <h2 className="text-[28px] sm:text-[30px] lg:text-[40px] leading-[1.25] text-[#111]">
              <span className="font-normal text-[#222]">{title}</span>
              <strong className="font-semibold text-black lg:mt-3 inline-block">{subtitle}</strong>
            </h2>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-y-12 sm:gap-y-16 lg:gap-y-0 lg:divide-x lg:divide-gray-200">
            {stats.map((stat: any, index: number) => (
              <StatItem key={index} {...stat} shouldAnimate={hasAnimated} />
            ))}
          </div>
        </div>
      </Wrapper>
