...
export default function UserSidebar({ isOpen = true, onToggle }: UserSidebarProps) {
  ...
  return (
    <aside className={`fixed inset-y-0 left-0 z-50 lg:relative h-screen flex flex-col shrink-0 font-sans bg-white dark:bg-[#0f1117] transition-all duration-300 ease-in-out overflow-hidden ${isOpen
                  ...
                  return (
                    <li key={item.label} className="relative">
                      ...
                      {item.href ? (
                        <Link
                          href={item.href}
                          className={`flex items-center gap-3.5 px-3.5 py-2.5 rounded-xl transition-colors ${active
                        ...
                        <button
                          ...
                          className={`w-full flex items-center justify-between px-3.5 py-2.5 rounded-xl transition-colors ${isParentActive
                          ...
                          {item.subItems.map((sub) => {
                            ...
                            return (
                              <li key={sub.label}>
                                <Link
                                  href={sub.href}
                                  className={`group flex items-center relative pl-[46px] pr-3 text-[13px] transition-colors ${isSubActive ? "text-black dark:text-white font-semibold" : "text-gray-400 dark:text-slate-500 hover:text-gray-700 dark:hover:text-slate-300 font-[400]"
                                    }`}
                                >
                                  <span className={`absolute left-[24px] w-[3px] h-[3px] rounded-full transition-colors ${isSubActive ? "bg-black dark:bg-white" : "bg-gray-300 dark:bg-slate-700 group-hover:bg-gray-400 dark:group-hover:bg-slate-500"
                                    }`}></span>
                                  {sub.label}
                                </Link>
                              </li>
